...
public final class SHA1
{
    ...
    void initMt()
    {
        calcthr = new Thread(new Runnable()
        {
    ...
    int[] _w = new int[w.length];

    boolean mt = true;

    private void perform_mt()
    {
        for (int t = 16; t < 80; t++)
        {
            int x = w[t - 3] ^ w[t - 8] ^ w[t - 14] ^ w[t - 16];
        ...
        System.arraycopy(w, 0, _w, 0, _w.length );
    ...
    ReentrantLock performLock = new ReentrantLock();
    ...
    void call_perform( int[] w )
    {
        ...
        //System.out.println("H:" + H0 + " " + H1+ " " + H2+ " " + H3+ " " + H4);
    }
    ...
    public static void main( String[] args ) throws NoSuchAlgorithmException, NoSuchProviderException
    {
         ...
         * test code.
         * H:-1449574858 1191608682 -1170332303 2018558572 -1664034659
H:-198678504 -1015339090 67696001 -2073595576 1247176050
H:-2070397372 473682542 -1162982751 -112121371 -448368399
         */
        ...
        MessageDigest tmd =  MessageDigest.getInstance("Tiger", "BC");


        MessageDigest smd =  MessageDigest.getInstance( "SHA-1");
        Whirlpool w = new Whirlpool();
        w.NESSIEinit();

        // THE WINNER IS:
        fr.cryptohash.Digest d = new fr.cryptohash.SHA1();
//        fr.cryptohash.Digest d = new fr.cryptohash.SHA1();
        ...
        boolean eq = hash_sha.length == hash_sun.length;
        if (eq)
        {
            for (int i = 0; i < hash_sun.length; i++)
            {
                if (hash_sun[i] != hash_sha[i])
                    eq = false;

            }
        }
        System.out.println("TMD took " + Long.toString( now2 - now) + "ms  Sun took " + Long.toString( now3 - now2) + "ms  SHA took " +
                Long.toString( now4 - now3) + "ms");
