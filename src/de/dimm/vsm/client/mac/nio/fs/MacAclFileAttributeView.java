...
package de.dimm.vsm.client.mac.nio.fs;
    ...
    
{
    ...
    // kauth.h KAUTH_WKG*
    private static final short ACE_OWNER = 		1;
private static final short ACE_GROUP = 		2;
private static final short KAUTH_WKG_NOBODY = 	3;
private static final short ACE_EVERYONE = 	4;

private static final int KAUTH_ACE_KINDMASK = 		0xf;
private static final int KAUTH_ACE_PERMIT = 		1;
private static final int KAUTH_ACE_DENY = 			2;
private static final int KAUTH_ACE_AUDIT = 			3;	/* not implemented */
private static final int KAUTH_ACE_ALARM = 			4;	/* not implemented */
private static final int KAUTH_ACE_INHERITED = 		(1<<4);
private static final int KAUTH_ACE_FILE_INHERIT = 		(1<<5);
private static final int KAUTH_ACE_DIRECTORY_INHERIT = 	(1<<6);
private static final int KAUTH_ACE_LIMIT_INHERIT = 		(1<<7);
private static final int KAUTH_ACE_ONLY_INHERIT = 		(1<<8);
private static final int KAUTH_ACE_SUCCESS = 		(1<<9);	/* not implemented (AUDIT/ALARM) */
private static final int KAUTH_ACE_FAILURE = 		(1<<10);	/* not implemented (AUDIT/ALARM) */


private static final int KAUTH_VNODE_READ_DATA = 			(1<<1);
private static final int KAUTH_VNODE_LIST_DIRECTORY = 		KAUTH_VNODE_READ_DATA;
private static final int KAUTH_VNODE_WRITE_DATA = 			(1<<2);
private static final int KAUTH_VNODE_ADD_FILE = 			KAUTH_VNODE_WRITE_DATA;
private static final int KAUTH_VNODE_EXECUTE = 			(1<<3);
private static final int KAUTH_VNODE_SEARCH = 			KAUTH_VNODE_EXECUTE;
private static final int KAUTH_VNODE_DELETE = 			(1<<4);
private static final int KAUTH_VNODE_APPEND_DATA = 			(1<<5);
private static final int KAUTH_VNODE_ADD_SUBDIRECTORY = 		KAUTH_VNODE_APPEND_DATA;
private static final int KAUTH_VNODE_DELETE_CHILD = 		(1<<6);
private static final int KAUTH_VNODE_READ_ATTRIBUTES = 		(1<<7);
private static final int KAUTH_VNODE_WRITE_ATTRIBUTES = 		(1<<8);
private static final int KAUTH_VNODE_READ_EXTATTRIBUTES = 		(1<<9);
private static final int KAUTH_VNODE_WRITE_EXTATTRIBUTES = 		(1<<10);
private static final int KAUTH_VNODE_READ_SECURITY = 		(1<<11);
private static final int KAUTH_VNODE_WRITE_SECURITY = 		(1<<12);
private static final int KAUTH_VNODE_TAKE_OWNERSHIP = 		(1<<13);
private static final int KAUTH_VNODE_CHANGE_OWNER = 		KAUTH_VNODE_TAKE_OWNERSHIP;
private static final int KAUTH_VNODE_SYNCHRONIZE = 			(1<<20);
private static final int KAUTH_VNODE_LINKTARGET = 			(1<<25);
private static final int KAUTH_VNODE_CHECKIMMUTABLE = 		(1<<26);
private static final int KAUTH_VNODE_ACCESS = 			(1<<31);
private static final int KAUTH_VNODE_NOIMMUTABLE = 			(1<<30);
private static final int KAUTH_VNODE_SEARCHBYANYONE = 		(1<<29);



//TODO: FIND CORRECT VALUE:

private static final int ACE_IDENTIFIER_GROUP = 0x0040;

    private final File file;
    private final boolean followLinks;

    MacAclFileAttributeView(File file, boolean followLinks) {
        this.file = file;
...
//        }
    ...
    private static void encode(List<AclEntry> acl, long address) {
        ...
        for (AclEntry ace: acl) {
            ...
            switch (ace.type()) {
                case ALLOW:
                    type = KAUTH_ACE_PERMIT;
                    break;
                case DENY:
                    type = KAUTH_ACE_DENY;
                    break;
                case AUDIT:
                    type = KAUTH_ACE_AUDIT;
                    break;
                case ALARM:
                    type = KAUTH_ACE_ALARM;
                    break;
                default:
...
//
    ...
    public UserPrincipal getOwner() throws IOException
    {
        ...
        int uid = PosixWrapper.getPosix().getuid();

        return UnixUserPrincipals.fromUid(uid);
    }
    ...
    public void setOwner(UserPrincipal owner) throws IOException {
        ...
        if (followLinks)
        {
            PosixWrapper.getPosix().lchown(file.getAbsolutePath(), uid, -1);
        } else {
            PosixWrapper.getPosix().chown(file.getAbsolutePath(),  uid, -1);
        }
    }
}
