...
class UnixUserPrincipals {
    ...
    static User fromUid(int uid) {
        String name = null;
        try {
            name = PosixWrapper.getPosix().getpwuid(uid).getLoginName();
    ...
    static Group fromGid(int gid) {
        String name = null;
        try {
            name = PosixWrapper.getPosix().getgrgid(gid).getName();
//            name = new String(getgrgid(gid));
        ...
        throws IOException
    {
        ...
        int id = -1;
        try {
            id = (isGroup) ? (int)PosixWrapper.getPosix().getgrnam(name).getGID() : (int)PosixWrapper.getPosix().getpwnam(name).getUID();
//            id = (isGroup) ? getgrnam(name) : getpwnam(name);
        } catch (Exception x) {
            throw new IOException(name + ": " + x.getMessage());
        }
        if (id == -1) {
