...
class _ReadRunner implements Runnable
{
    final MultiThreadedFileReaderSave aiw;

    public _ReadRunner( MultiThreadedFileReaderSave aiw )
    {
        this.aiw = aiw;
    }
        ...
        while (aiw.keepRunning)
        {
            try
            {
                ...
                if (elem != null)
                {
                    ...
                    if (MultiThreadedFileReaderSave.verbose)
                        System.out.println("Taking lock from queue " + lock.toString() );
                    b = System.currentTimeMillis();
...
class _HashRunner implements Runnable
{
    final MultiThreadedFileReaderSave aiw;


    public _HashRunner( MultiThreadedFileReaderSave aiw )
    {
        this.aiw = aiw;        
    }
    ...
    public void run()
    {
        ...
        while (aiw.keepRunning)
        {
            try
            {
                ...
                long a = System.currentTimeMillis();
                FileCacheElem fce = aiw.hashQueue.poll(aiw.sleepMilisecondOnEmpty, TimeUnit.MILLISECONDS);
                long b = System.currentTimeMillis();
                ...
                if (fce != null)
                {
                    try
                    {
                        ...
                        if (MultiThreadedFileReaderSave.verbose)
                            System.out.println("finished hash locking and signalling " + fce.lock.toString());
                        fce.lock.readyLock.lock();
                        fce.hashready = true;
                        fce.lock.hashReady.signal();
                    }
                    finally
                    {
                        fce.lock.readyLock.unlock();
                        if (MultiThreadedFileReaderSave.verbose)
                            System.out.println("finished hash release " + fce.lock.toString());
                    }                    
                }
                else
                {
...
class _FinishRunner implements Runnable
{
    final MultiThreadedFileReaderSave aiw;


    public _FinishRunner( MultiThreadedFileReaderSave aiw )
    {
        this.aiw = aiw;
    }
...
class _HashReadyLock
{
    int idx;
    ReentrantLock readyLock;
    Condition hashReady;

    public _HashReadyLock( int idx)
    {
        this.idx = idx;
...
public abstract class MultiThreadedFileReaderSave
{
   ...
   // HashFunctionPool hashPool;
    ...
    public MultiThreadedFileReaderSave( int maxElems)
    {
    ...
    public static MultiThreadedFileReaderSave MTFRFactory(NetAgentApi api )
    {
...
//            return new WinMultiThreadedFileReader();
    ...
    {
        _ReadRunner wr = new _ReadRunner(this);
        writerThread = new Thread(wr, "ReadRunner");
        writerThread.start();
        for (int i = 0; i < hashThreads.length; i++)
        {
            _HashRunner hr = new _HashRunner(this);
            hashThreads[i] = new Thread(hr, "HashRunner");
            hashThreads[i].start();
        }
        _FinishRunner fr = new _FinishRunner(this);
